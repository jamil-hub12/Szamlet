...
export function NotificacionesProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code === "PGRST205") {
            console.warn("⚠️ La tabla 'notificaciones' no existe. Ejecuta el script SQL en Supabase:");
  ...
  const agregarNotificacion = async (notif: Omit<Notificacion, "id" | "timestamp" | "leida">) => {
    try {
  ...
  const enviarEmailCambioEstado = async (data: {
    ...
    if (!isEmailJSConfigured()) {
      console.warn("⚠️ EmailJS no está configurado. Edita src/config/emailjs.config.ts");
      return false;
    }

    try {
      console.log("📧 Enviando email de cambio de estado...", data);
