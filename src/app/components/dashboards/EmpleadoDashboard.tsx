...
import { NuevoPedidoModal, type NuevoPedidoOutput } from "../pedidos/NuevoPedidoModal";
...
import { RegistrarPagoModal } from "../pedidos/RegistrarPagoModal";
...
> = {
  Recibido: {
    color: `${obtenerEstiloEstado("Recibido").bgColor} ${obtenerEstiloEstado("Recibido").color} ${obtenerEstiloEstado("Recibido").borderColor}`,
    icon: <Clock className="w-3 h-3" />,
  },
  "En confección": {
    color: `${obtenerEstiloEstado("En confección").bgColor} ${obtenerEstiloEstado("En confección").color} ${obtenerEstiloEstado("En confección").borderColor}`,
    icon: <Loader2 className="w-3 h-3 animate-spin" />,
  },
  "Listo para entrega": {
    color: `${obtenerEstiloEstado("Listo para entrega").bgColor} ${obtenerEstiloEstado("Listo para entrega").color} ${obtenerEstiloEstado("Listo para entrega").borderColor}`,
    icon: <Package className="w-3 h-3" />,
  },
  Entregado: {
    color: `${obtenerEstiloEstado("Entregado").bgColor} ${obtenerEstiloEstado("Entregado").color} ${obtenerEstiloEstado("Entregado").borderColor}`,
    icon: <CheckCircle2 className="w-3 h-3" />,
  },
  Cancelado: {
    color: `${obtenerEstiloEstado("Cancelado").bgColor} ${obtenerEstiloEstado("Cancelado").color} ${obtenerEstiloEstado("Cancelado").borderColor}`,
    icon: <XCircle className="w-3 h-3" />,
  },
  Vencido: {
    color: `${obtenerEstiloEstado("Vencido").bgColor} ${obtenerEstiloEstado("Vencido").color} ${obtenerEstiloEstado("Vencido").borderColor}`,
    icon: <XCircle className="w-3 h-3" />,
  },
};
...
// (tipo Cliente importado de ClientesContext, ver imports al inicio del archivo)
    ...
    if (exito) {
      ...
      agregarNotificacion({
        ...
        mensaje: generarMensajeCambioEstado(
          estadoAnterior as EstadoPedido,
          siguiente,
                    ...
                    <tbody>
                      {pedidos
                        .filter((p) => {
                          ...
                          const montoTotal = p.montoTotal || 0;
                          const montoPagado = p.montoPagado || 0;
                          const montoPendiente = calcularSaldoPendiente(
                            montoTotal,
                            montoPagado,
                          );
                        ...
                        .sort((a, b) => {
                          const pendienteA = calcularSaldoPendiente(
                            a.montoTotal,
                            a.montoPagado,
                          );
                          const pendienteB = calcularSaldoPendiente(
                            b.montoTotal,
                            b.montoPagado,
                          );
