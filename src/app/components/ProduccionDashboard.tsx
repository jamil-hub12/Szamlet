...
export function ProduccionDashboard() {
  ...
  const productosFiltrados = productos.filter((p) => {
    ...
    if (filtro === "incompletos") return match && tieneStockFaltante(p);
    if (filtro === "completos") return match && !tieneStockFaltante(p);
    return match;
  });
  ...
  const diasParaEntrega = (fecha: string | null): number | null => {
            ...
            {cargando ? (
              ...
              <div className="space-y-3">
                {solicitudesFiltradas.map((solicitud) => {
                  ...
                  const dias = diasParaEntrega(solicitud.fechaEntrega);
